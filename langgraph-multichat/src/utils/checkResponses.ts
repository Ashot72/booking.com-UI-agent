...
export function ensureToolCallHaveResponses(messages: Message[]): Message[] {
    const newMessages: ToolMessage[] = []

    messages.forEach((message, index) => {
        if (message.type !== "ai" || message.tool_calls?.length === 0) {
            //If it is not an AI message, or it does not have tool calls, we can ignore
            return
        }
    ...
    /*
      ...
      // the last call is human call
      const messages = [
        { type: "ai", tool_calls: [{ id: "abc", name: "getWeather" }] },
